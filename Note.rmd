...
county.2006 <- X2006_results %>%
  ...
  summarise(reg = sum(as.numeric(reg), na.rm = TRUE),
            turnout = sum(as.numeric(Signature), na.rm = TRUE),
            CongIR = sum(as.numeric(CongR), na.rm = TRUE),
            CongDFL = sum(as.numeric(CongDFL), na.rm = TRUE),
            CongOther = sum(as.numeric(CongIP), na.rm = TRUE) + sum(as.numeric(CongWI), na.rm = TRUE) + sum(as.numeric(CongTOT), na.rm = TRUE), 
            GovIR = sum(as.numeric(GovR), na.rm = TRUE),
            GovDFL = sum(as.numeric(GovDFL), na.rm = TRUE),
            GovOther = sum(as.numeric(GovIP), na.rm = TRUE) + sum(as.numeric(GovWI), na.rm = TRUE) + sum(as.numeric(GovTOT), na.rm = TRUE)
            )
...
county.2008 <- X2008_results %>%
  ...
  summarise(reg = sum(reg),
            ...
            PresIR = sum(USPRESR),
            PresDFL = sum(USPRESDFL),
            PresOther = sum(USPRESGP) + sum(USPRESSWP) + sum(USPRESIND) + sum(USPRESLIB) + sum(USPRESCP) + sum(USPRESWI) + sum(USPRESTOT),
            CongIR = sum(CONGR),
            CongDFL = sum(CONGDFL)#,
            #CongOther = sum(USCongI) + sum(USCongWI)
            )
